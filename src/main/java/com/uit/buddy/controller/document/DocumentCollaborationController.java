...
public class DocumentCollaborationController extends AbstractBaseController {
    ...
    public ResponseEntity<SingleResponse<DocumentContentResponse>> getDocumentContent(
    ...
    public ResponseEntity<SingleResponse<DocumentContentResponse>> updateDocumentContent(
            @PathVariable UUID documentId,
            @Valid @RequestBody UpdateDocumentContentRequest request,
            @AuthenticationPrincipal String mssv) {

        log.info("[PATCH /api/documents/{}/content] Updating document content for mssv: {}, version: {}",
                documentId, mssv, request.getVersion());

        DocumentContentResponse response = documentCollaborationService.updateDocumentContent(
                documentId, request, mssv);
