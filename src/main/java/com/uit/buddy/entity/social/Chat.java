...
public class Chat extends AbstractBaseEntity {
    ...
    @JoinColumn(name = "group_id", referencedColumnName = "id", foreignKey = @ForeignKey(name = "fk_chat_group"))
    private Group group;
    ...
    @JoinColumn(name = "mssv_sender", referencedColumnName = "mssv", nullable = false, foreignKey = @ForeignKey(name = "fk_chat_sender"))
    ...
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "mssv_receiver", referencedColumnName = "mssv", foreignKey = @ForeignKey(name = "fk_chat_receiver"))
