...
public class AuthServiceImpl implements AuthService {
  ...
  public AuthResponse completeSignUp(CompleteSignUpRequest request) {
    ...
    if (!pendingAccount.getMssv().equals(request.mssv())) {
      throw new AuthException(
          AuthErrorCode.INVALID_CREDENTIALS, "Invalid signup request. MSSV does not match.");
    }
    ...
    try {
      ...
      log.info(
          "Successfully created student: {} with homeClassCode: {}", request.mssv(), homeClassCode);
    } catch (Exception e) {
      ...
      throw new AuthException(
          AuthErrorCode.EXTERNAL_SERVICE_ERROR, "Failed to create account. Please try again.");
    }
  ...
  public AuthResponse signIn(SignInRequest request) {
    ...
    String decryptedWstoken =
        wsTokenEncryptionService.decryptWstoken(student.getEncryptedWstoken());

    try {
      uitClient.fetchSiteInfo(decryptedWstoken);
    } catch (ExternalClientException e) {
      log.warn(
          "Wstoken invalid for MSSV: {}. User needs to re-authenticate with Moodle",
          request.mssv());
      throw new AuthException(
          AuthErrorCode.INVALID_WSTOKEN, "Your Moodle token has expired. Please re-authenticate.");
    }
  ...
  public void forgetPassword(ForgetPasswordRequest request) {
    ...
    try {
      ...
      throw new AuthException(
          AuthErrorCode.EMAIL_SEND_FAILED, "Failed to send OTP email. Please try again.");
    } catch (Exception e) {
  ...
  public AuthResponse refreshToken(String refreshToken) {
    ...
    if (!refreshTokenRepository.existsById(refreshToken)) {
      ...
      throw new AuthException(
          AuthErrorCode.REFRESH_TOKEN_EXPIRED, "Refresh token has been revoked or expired");
    }
  ...
  private void ensureHomeClassExists(String homeClassCode, String mssv) {
    ...
    if (homeClassCode == null || homeClassCode.length() < 4) {
      ...
      throw new HomeClassException(
          HomeClassErrorCode.INVALID_HOME_CLASS_FORMAT, "Mã lớp không hợp lệ.");
    }
    ...
    if (!majorRepository.existsById(majorCode)) {
      cometChatClient.deleteUser(mssv);
      throw new HomeClassException(
          HomeClassErrorCode.MAJOR_NOT_FOUND, "Ngành " + majorCode + " chưa có trong hệ thống.");
    }
  ...
  private void createCometChatUser(String mssv, String fullName, String avatarUrl) {
    log.debug(
        "[Auth Service] Preparing CometChat request - mssv: {}, fullName: {}, avatarUrl: {}",
        mssv,
        fullName,
        avatarUrl);

    try {
      CometChatUserRequest cometChatRequest =
          new CometChatUserRequest(mssv, fullName != null ? fullName : mssv, avatarUrl);
      ...
      if (e.getMessage() != null && e.getMessage().contains("already exists")) {
        log.warn(
            "[Auth Service] CometChat user already exists for MSSV: {}. Continuing signup...",
            mssv);
        // User already exists in CometChat, continue with signup
        return;
      }
      log.error(
          "[Auth Service] CometChat integration failed for MSSV: {}. Error: {}",
          mssv,
          e.getMessage());
      throw new AuthException(
          AuthErrorCode.EXTERNAL_SERVICE_ERROR, "Failed to initialize chat service");
    } catch (RestClientException e) {
      log.error(
          "[Auth Service] CometChat connection failed for MSSV: {}. Error: {}",
          mssv,
          e.getMessage());
      throw new AuthException(
          AuthErrorCode.EXTERNAL_SERVICE_ERROR, "Failed to connect to chat service");
    }
  }
  ...
  private String extractHomeClassCode(String wstoken, Long userid, String mssv) {
    try {
      ...
      return courses.stream()
          .filter(
              course ->
                  course.fullName() != null && course.fullName().toUpperCase().contains("CVHT"))
  ...
  private SiteInfoResponse validateMoodleToken(String wstoken) {
    ...
    if (siteInfo.username() == null || siteInfo.username().isBlank()) {
      throw new AuthException(
          AuthErrorCode.INVALID_CREDENTIALS, "Cannot extract MSSV from Moodle response");
    }
