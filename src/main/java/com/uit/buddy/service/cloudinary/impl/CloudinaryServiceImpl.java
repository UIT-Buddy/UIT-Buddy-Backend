...
public class CloudinaryServiceImpl implements CloudinaryService {
  ...
  public String createDefaultAvatar(String mssv) {
    return executeAvatarUpload(
        properties.getDefaultAvatarUrl(),
        mssv,
  ...
  public void deleteAvatar(String publicId) {
    executeDelete(
        CloudinaryConstants.FOLDER_AVATARS + "/" + publicId,
        CloudinaryConstants.RESOURCE_TYPE_IMAGE);
  }
  ...
  public void validateFile(MultipartFile file, FileType fileType) {
    ...
    if (file == null || file.isEmpty()) throw new UserException(UserErrorCode.FILE_EMPTY);

    if (file.getContentType() == null
        || Arrays.stream(allowed).noneMatch(file.getContentType()::equalsIgnoreCase)) {
      throw new UserException(
          UserErrorCode.INVALID_FILE_TYPE, "Allowed: " + String.join(", ", allowed));
    }
  }

  private PostMedia executeUpload(
      Object source,
      String publicId,
      String folder,
      String resType,
    ...
    try {
      ...
      if (trans != null) params.put(CloudinaryConstants.PARAM_TRANSFORMATION, trans);
  ...
  private String executeAvatarUpload(
      Object source, String publicId, String folder, String resType, Transformation<?> trans) {
    try {
      Map<String, Object> params = new HashMap<>();
      params.put(CloudinaryConstants.PARAM_PUBLIC_ID, publicId);
      params.put(CloudinaryConstants.PARAM_FOLDER, folder);
