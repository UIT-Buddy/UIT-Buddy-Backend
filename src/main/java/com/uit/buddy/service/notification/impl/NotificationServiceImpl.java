...
public class NotificationServiceImpl implements NotificationService {
    ...
    public void createPostLikeNotification(PostLikedEvent event) {
        ...
        processNotification(
                event.receiverMssv(),
                template.getTitle(),
                template.formatContent(event.actorName()),
                template.getType(),
    ...
    public void createPostCommentNotification(PostCommentedEvent event) {
        ...
        processNotification(
                event.receiverMssv(),
                template.getTitle(),
                template.formatContent(event.actorName(), event.commentContent()),
                template.getType(),
                event.postId().toString(),
                "/posts/" + event.postId());
    }
    ...
    public void createPostShareNotification(PostSharedEvent event) {
        ...
        processNotification(
                ...
                template.formatContent(event.actorName()),
                template.getType(),
                event.originalPostId().toString(),
                "/posts/" + event.originalPostId());
    }

    private void processNotification(String receiverMssv, String title, String content,
            String type, String dataId, String redirectUrl) {

        Notification notification = Notification.builder()
                .student(studentRepository.getReferenceById(receiverMssv))
                .title(title)
                .content(content)
